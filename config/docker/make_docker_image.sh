...
offline_path="${current_path}/offline"
...
export_docker() {
    ...
    local docker_image_name="openwrt-docker-${docker_os}-arm64"
    local full_image="${docker_image_name}:arm64"

    # Build docker image locally
    cd ${out_path}
    docker build --platform linux/arm64 -t "${full_image}" .
    ...
    rm -rf ${offline_path} && mkdir -p ${offline_path}
    local export_file="${offline_path}/${docker_image_name}.tar.gz"
