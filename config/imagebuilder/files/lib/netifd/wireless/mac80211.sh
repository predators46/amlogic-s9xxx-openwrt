...
mac80211_prepare_vif() {
	...
	case "$mode" in
		monitor|mesh)
			[ "$auto_channel" -gt 0 ] || ifconfig dev "$ifname" set channel "$channel" $iw_htmode
		;;
	esac
...
mac80211_setup_adhoc() {
	...
	ifconfig dev "$ifname" ibss join "$ssid" $freq $iw_htmode fixed-freq $bssid \
...
mac80211_setup_mesh() {
	...
	ifconfig dev "$ifname" mesh join "$ssid" freq $freq $iw_htmode \
		${mcval:+mcast-rate $mcval} \
		beacon-interval $beacon_int
}
